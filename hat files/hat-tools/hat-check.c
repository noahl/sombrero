...
/*   Further changes that would be good - remove assumption that sizeof(char)
 *   is one byte. Add range checks (for -n <offset>, the file size, maybe other
 *   things too). Look at readfloat() and readdouble(), maybe set definitive
 *   size (right now hat files are not portable across platforms with different
 *   sizeof(float) and sizeof(double)). make sure (unsigned long) is at least
...
reportstats (void)
{
  ...
  printf("%10u   %-20s%12u%10.1f\n",
    grandcount, "whole trace file", grandspace, pc((unsigned long)grandspace,
                                                   (unsigned long)grandspace));
}
...
header (void)
{
  ...
  if (amode) printf("\nError message: ");
  dopointer(MAYBEZERO, AtomAbstract, readpointer(), HEADER, (FileOffset)0, "");
  if (amode) printf("\n");
