...
# our environment stores 3-tuples of (value, trace of ConstDef, trace of value)
...
def assign(name, val): # val is a 3-tuple
	for i in xrange(len(frames)-1, -1, -1):
		if name in frames[i]:
			frames[i][name] = val
			return
...
def traced_assign(asn, parent):
	...
	# NOTE FOR ASSIGNS: right now, we're pretending the assign itself
	# doesn't exist, and just counting it as the right hand side. It might
	# be good to make a dummy function that we can say we're going to pass
	# the value through. (that's also consistent with counting them as
	# projections.)
	...
	assert all([name.__class__ == _ast.Name for name in asn.targets])
	traces = [mkConstDef(mkRoot(),
	...
	def ev():
		...
		for (name, cdef) in zip(asn.targets, traces):
			entResult(cdef, 0)
			assign(name.id, (valr[0], cdef, valr[1]))
			resResult(cdef, valr[1], 0)
		
		return (None, None) # what *should* the trace value here be?
	
	return (ev, None)
	# this return is wrong. the right solution is to make statements return
	# either only a thunk, or a list of traces. but this is fine for now.
...
def traced_boolop(boolop, parent):
	...
	if boolop.op.__class__ == _ast.And:
		subs = map(lambda (x): exec_tracing(x, parent), boolop.values)
		app = mkAppn(parent, 0, primitive_and(), len(boolop.values), *tuple(map(lambda (x): x[1], subs)))
		def ev():
			entResult(app, 0)
			for this in subs:
				val = this[0]()
		...
		subs = map(lambda (x): exec_tracing(x, parent), boolop.values)
		app = mkAppn(parent, 0, primitive_or(), len(boolop.values), *tuple(map(lambda (x): x[1], subs)))
		def ev():
			entResult(app, 0)
			for this in subs:
				val = this[0]()
...
def traced_name(name, parent):
	...
	var = lookup(name.id) # var is (val, trace of constdef, trace of val)
	use = mkConstUse(parent, 0, var[1]) 
	def ev():
		return (var[0], use)
	return (ev, use)
...
def trace_file(filename):
	...
	try:
		...
		for stmt in mod.body[:-1]:
			exec_tracing(stmt, definition_main)[0]()
