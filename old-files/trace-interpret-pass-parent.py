...
# the environment stores pairs of (value, trace of ConstDef)
...
def assign(name, val): # val is a pair
	for i in xrange(len(frames)-1, -1, -1):
		if name in frames[i]:
			frames[i][name] = val
			return
...
def access(name, parent):
	try:
		(val, constdef) = lookup(name)
		constuse = mkConstUse (parent, 0, constdef)
		return (val, constuse)
	except NameError:
...
# exec_tracing, and all of its subroutines, trace in two steps: first, they
...
def exec_tracing(ast, parent):
	...
	assert isinstance(parent, int) or isinstance(parent, long) # int should be FileOffset here
	
	if ast.__class__ == _ast.Expr: # an Expr is a type of stmt.
		res = eval_tracing(ast.value, parent)
		res[0]()
	elif ast.__class__ == _ast.Assign:
		exec_assign(ast, parent)
	elif ast.__class__ == _ast.FunctionDef:
		exec_functiondef(ast, parent)
...
def eval_tracing(ast, parent):
	assert isinstance(ast, _ast.expr)
	assert isinstance(parent, int) or isinstance(parent, long) # these types are actually FileOffset
	
	if ast.__class__ == _ast.BinOp:
		res = eval_binop(ast, parent)
	elif ast.__class__ == _ast.BoolOp:
		res = eval_boolop(ast, parent)
	elif ast.__class__ == _ast.Call:
		res = eval_call(ast, parent)
	elif ast.__class__ == _ast.Name:
		res = eval_name(ast, parent)
	elif ast.__class__ == _ast.Num:
		res = eval_num(ast, parent)
	elif ast.__class__ == _ast.UnaryOp:
		res = eval_unaryop(ast, parent)
	else:
		raise Exception, ("Unrecognized AST node %s!" % (ast.__class__.__name__,))
...
def exec_assign(asn, parent):
	...
	val = eval_tracing(asn.value, trace)
...
def eval_binop(binop, parent):
	assert binop.__class__ == _ast.BinOp
	
	left = eval_tracing(binop.left, parent)
	right = eval_tracing(binop.right, parent)
	
	if binop.op.__class__ == _ast.Add:
		app = mkApp2(parent,
		             0,
		             mkValueUse(parent, 0, primitive_add()),
		             left[1],
		             right[1])
		def ev():
			entResult(app, 0)
		...
		app = mkApp2(parent,
		             0,
		             mkValueUse(parent, 0, primitive_sub()),
		             left[1],
		             right[1])
		def ev():
			entResult(app, 0)
		...
		app = mkApp2(parent,
		             0,
		             mkValueUse(parent, 0, primitive_mul()),
		             left[1],
		             right[1])
		def ev():
			entResult(app, 0)
		...
		app = mkApp2(parent,
		             0,
		             mkValueUse(parent, 0, primitive_div()),
		             left[1],
		             right[1])
		def ev():
			entResult(app, 0)
...
def eval_boolop(boolop, parent):
	...
	if boolop.op.__class__ == _ast.And:
		subs = map(lambda (x): eval_tracing(x, parent), boolop.values)
		app = mkAppn(parent,
		             0, 
		             mkValueUse(parent, 0, primitive_and()),
		             len(boolop.values),
		...
		subs = map(lambda (x): eval_tracing(x, parent), boolop.values)
		app = mkAppn(parent,
		             0, mkValueUse(parent, 0, primitive_or()),
		             len(boolop.values),
		             *tuple(map(lambda (x): x[1], subs)))
		def ev():
			entResult(app, 0)
...
def eval_call(call, parent):
	...
	if (len(call.keywords) == 0
		...
		func = eval_tracing(call.func, parent)
		args = map(lambda(x):eval_tracing(x, parent), call.args)
		
		funcr = func[0]()[0]
		argsr = map(lambda(x): x[0]()[0], args)
		
		if funcr.__class__ == traced_function:
			raise Exception, "eval_call can't do traced functions yet!"
		elif callable(funcr): # calling an unwrapped function
			app = mkAppn(parent, 0, func[1], len(args), *tuple(map(lambda(x):x[1], args)))
			def ev():
				entResult(app, 0)
...
def exec_functiondef(fdef, parent):
...
def exec_if(exp, parent):
	assert exp.__class__ == _ast.If
	
	cond = eval_tracing(exp.test, parent)
	...
	if condr[0]:
		for stmt in exp.body:
			exec_tracing(stmt, trace) # ifs are parents
	else:
		for stmt in exp.orelse:
			exec_tracing(stmt, trace) # ifs are parents
	
	resResult(trace, mkValueUse(parent, 0, primitive_none()), 0)
	
	return None

def eval_name(name, parent):
	assert name.__class__ == _ast.Name
	
	# name: the only trace that doesn't compute a result?
	# no - literals don't either
	
	(val, trace) = access(name.id, parent)
	def ev():
		return (val, trace)
	return (ev, trace)

def eval_num(num, parent):
	assert num.__class__ == _ast.Num
	assert isinstance(num.n, int)
	
	trace = mkInt(parent, 0, num.n)
	def ev():
		return (num.n, trace)
	return (ev, trace)
...
def eval_unaryop(unaryop, parent):
	assert unaryop.__class__ == _ast.UnaryOp
	
	operand = eval_tracing(unaryop.operand, parent)
	
	if unaryop.op.__class__ == _ast.Not:
		app = mkApp1(parent,
		             0,
		             mkValueUse(parent, 0, primitive_not()),
		             operand[1])
		def ev():
			entResult(app, 0)
...
def trace_file(filename):
	...
	try:
		...
		assert mod.body[len(mod.body)-1].__class__ == _ast.Expr
		res = eval_tracing(mod.body[len(mod.body)-1].value, definition_main)
		res[0]() # experiment!
